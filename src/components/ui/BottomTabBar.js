    ...
    <View style={styles.container}>
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            <IconeComponente name={tab.icon} size={tab.size ?? 26} color={ativa ? colors.primary : '#000'} />
...
const styles = StyleSheet.create({
  container: { flexDirection: 'row', backgroundColor: colors.surface, paddingVertical: spacing.sm, borderTopWidth: 1, borderTopColor: colors.border, justifyContent: 'space-around' },
  item: { alignItems: 'center', flex: 1 },
  label: { fontSize: 11, color: '#000', marginTop: 4 },
